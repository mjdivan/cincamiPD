...
/**
 * It is an especialization of the attribute class for describing the context properties.
 * @author Mario Divan
 * @version 1.0
 */
@XmlRootElement(name="ContextProperty")
@XmlType(propOrder={"relevance"})
public class ContextProperty extends org.ciedayap.ipd.requirements.Attribute
        implements Serializable, Level
{
    ...
    public static synchronized ContextProperty create(String ID, String nam, BigDecimal we, Metrics m,BigDecimal rel)            
    {
        ContextProperty a=new ContextProperty();
        a.setWeight(we);
        a.setID(ID);
        a.setName(nam);
        a.setMetrics(m);
    ...
    public String writeTo() throws ProcessingException {
        ...
        sb.append(Tokens.startLevel).append(Tokens.Class_ID_CtxProperty);
        ...
        //"objective" *optional*
        sb.append((StringUtils.isEmpty(this.getObjective()))?"":Tokens.removeTokens(this.getObjective())).append(Tokens.fieldSeparator);        
        //weight *mandatory*
        sb.append(this.getWeight()).append(Tokens.fieldSeparator);
        //minEstimatedValue *optional*
        sb.append((this.getMinEstimatedValue()==null)?"":this.getMinEstimatedValue()).append(Tokens.fieldSeparator);
        //maxEstimatedValue *optional*
        sb.append((this.getMaxEstimatedValue()==null)?"":this.getMaxEstimatedValue()).append(Tokens.fieldSeparator);
        //avgEstimatedValue *optional*
        sb.append((this.getAvgEstimatedValue()==null)?"":this.getAvgEstimatedValue()).append(Tokens.fieldSeparator);
        //sdEstimatedValue *optional*
        sb.append((this.getSdEstimatedValue()==null)?"":this.getSdEstimatedValue()).append(Tokens.fieldSeparator);
        //kurtosisEstimatedValue *optional*
        sb.append((this.getKurtosisEstimatedValue()==null)?"":this.getKurtosisEstimatedValue()).append(Tokens.fieldSeparator);
        //skewnessEstimatedValue *optional*
        sb.append((this.getSkewnessEstimatedValue()==null)?"":this.getSkewnessEstimatedValue()).append(Tokens.fieldSeparator);
        //medianEstimatedValue *optional*
        sb.append((this.getMedianEstimatedValue()==null)?"":this.getMedianEstimatedValue()).append(Tokens.fieldSeparator);
        //modeEstimated *optional*
        sb.append((this.getModeEstimated()==null)?"":this.getModeEstimated().writeTo()).append(Tokens.fieldSeparator);
        //relevance *mandatory*
        sb.append(relevance).append(Tokens.fieldSeparator);
        //metrics *mandatory*
        sb.append(this.getMetrics().writeTo());
                
        sb.append(Tokens.Class_ID_CtxProperty).append(Tokens.endLevel);
    ...
    public static ContextProperty readFromSt(String text) throws ProcessingException {
        if(StringUtils.isEmpty(text)) return null;
        
        int idx_st=text.indexOf(Tokens.startLevel+Tokens.Class_ID_CtxProperty);
        int idx_en=text.lastIndexOf(Tokens.Class_ID_CtxProperty+Tokens.endLevel);
        if(idx_st<0 || idx_en<0) return null;
        
        ContextProperty item=new ContextProperty();
        
        //It removes the startLevel and endLevel characters
        String cleanedText=text.substring(idx_st+Tokens.Class_ID_CtxProperty.length()+1, idx_en);                
        ...
        //relevance *mandatory*               
        idx_en=cleanedText.indexOf(Tokens.fieldSeparator);
        if(idx_en<0) throw new ProcessingException("The relevance field separator is not present.");
        value=cleanedText.substring(0, idx_en);
        if(StringUtils.isEmpty(value)) throw new ProcessingException("The relevance field is not present and it is mandatory");
        item.setRelevance(new BigDecimal(value));
    ...
    public boolean isDefinedProperties() {
        return !(StringUtils.isEmpty(this.getID()) || StringUtils.isEmpty(name) || weight==null ||
                weight.compareTo(BigDecimal.ZERO)<0 || weight.compareTo(BigDecimal.ONE)>0 ||
                relevance==null || !this.getMetrics().isDefinedProperties());
    }
    
    public static ContextProperty test(int k)
    {
        ContextProperty at=new ContextProperty();
        at.setID("IDAtt"+k);
        ...
        at.setRelevance(BigDecimal.ONE);
    ...
    public static void main(String arg[]) throws ProcessingException
    {
        ...
        ContextProperty at2=ContextProperty.readFromSt(brief);
    ...
    public boolean equals(Object target)
    {
        ...
        if(!(target instanceof ContextProperty)) return false;
        
        ContextProperty cp=(ContextProperty)target;
    ...
    public ArrayList<TokenDifference> findDifferences(Object ptr) throws ProcessingException {
        ...
        if(!(ptr instanceof ContextProperty)) throw new ProcessingException("The instance to be compared is not of the expected type");
        ContextProperty comp=(ContextProperty)ptr;
        ...
        global.add(TokenDifference.create(ContextProperty.class, this.getUniqueID(), this.getLevel(), this.computeFingerprint(), comp.computeFingerprint()));
